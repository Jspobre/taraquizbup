...
class DoubleCircularNotchedButton extends NotchedShape {
  ...
  Path getOuterPath(Rect host, Rect? guest) {
    ...
    ///Cut-out 1
    final List<Offset?> px = List<Offset?>.filled(6, null);
    ...
    for (int i = 0; i < px.length; i += 1) {
       px[i] = px[i]! + Offset(0 + (notchRadius + 12), 0); //Cut-out 1 positions
    } //Cut-out 1 positions

    ///Cut-out 2
    final List<Offset?> py = List<Offset?>.filled(6, null);
    ...
    for (int i = 0; i < py.length; i += 1) {
      py[i] = py[i]! + Offset(host.width - (notchRadius + 12), 0); //Cut-out 2 positions
    } //Cut-out 2 positions
