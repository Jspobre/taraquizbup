...
class _MultiQuizState extends State<MultiQuiz> {
  ...
  onpressed() {
    Audio.load(FileConstants.assetBuzzerAudio)
      ..play()
      ..dispose();
    if(dataMap["buzzer"]["buzzer1"] == "default" && dataMap["buzzer"]["buzzer1"] == "default"){
    ...
    else if(dataMap["buzzer"]["buzzer2"] == "default" && dataMap["buzzer"]["buzzer1"] != "default" && dataMap["buzzer"]["buzzer1"] != widget.uid){
      FirebaseDatabase.instance
          .ref(
          "quizRoom/${widget.proUid}/${widget.subjectCode}/${widget.testCode}/${widget.groupCode}")
          .update({
  ...
  Widget build(BuildContext context) {
            ...
            child: Card(
              child: Column(
                ...
                children: [
                  ...
                  (!dataMap["options"][countNotifier.value][5].toString().contains("(If any)")) ? Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: SizedBox(
                        width: MediaQuery.of(context).size.width * 70 / 100,
                        child: Image.network(dataMap["options"][countNotifier.value][5].toString(),
                            scale: 1)),
                  ) : SizedBox.shrink()
                ],
              ),
      ...
      _answerWidget() {
        if(countNotifier.value<20){
          if ( dataMap["type"][countNotifier.value].toString().split(" ")[0] == "MCQs") {
            return Card(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                      ...
                      child: Column(
                        children: [
                          RawChip(
                            ...
                            onSelected: (dataMap["buzzer"]["buzzer1"] == widget.uid || dataMap["buzzer"]["buzzer2"] == widget.uid) ? (bool selected) {
                              selected = true;
                              setState(() {
                                  _value = selected ? index : null;
                                  userAnwersList[countNotifier.value] = (index+1).toString();
                                  anwersList[countNotifier.value] = dataMap["options"][countNotifier.value][4];
                                });
                            } : null,
                          ),
                          (!dataMap["options"][countNotifier.value][6+index].toString().contains("(If any)")) ? Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: SizedBox(
                                width: MediaQuery.of(context).size.width * 70 / 100,
                                child: Image.network(dataMap["options"][countNotifier.value][6+index].toString(),
                                    scale: 1)),
                          ) : SizedBox.shrink()
                        ],
                      ),
          ...
          else if (dataMap["type"][countNotifier.value].toString().split(" ")[0] == "True/False")
          { return Wrap(
            children: List<Widget>.generate(
              2,
                  (int index) {
                  ...
                  child: RawChip(
                    ...
                    onSelected: (dataMap["buzzer"]["buzzer1"] == widget.uid || dataMap["buzzer"]["buzzer2"] == widget.uid) ? (bool selected) {
                      ...
                      setState((){
                         _value = selected ? index : null;
                         userAnwersList[countNotifier.value] = (index+1).toString();
                         anwersList[countNotifier.value] = dataMap["options"][countNotifier.value][0];
                       });
                         } : null,
                  ),
                );
          ...
          else if (dataMap["type"][countNotifier.value].toString().split(" ")[0] == "Fill")
          {return SizedBox(
            width: MediaQuery
                .of(context)
                .size
            ...
            child: TextField(
              ...
              maxLines: 4,
              enabled: (dataMap["buzzer"]["buzzer2"] == widget.uid || dataMap["buzzer"]["buzzer1"] == widget.uid),
        ...
        bottom: PreferredSize(
          ...
          child: Padding(
            ...
            child: ValueListenableBuilder(
                ...
                builder: (BuildContext context, String value, Widget? child) {  return Text(value, style: Theme.of(context)
                    .textTheme
                    .labelLarge
      ...
      timerEnded ? Container(
        ...
        child: StreamBuilder(
            stream: FirebaseDatabase.instance
                .ref()
                .child(
                'quizRoom/${widget.proUid}/${widget.subjectCode}/${widget
                    .testCode}/${widget.groupCode}')
                .onValue
                .asBroadcastStream(),
            builder: (context, dbEvent) {
              if (dbEvent.connectionState == ConnectionState.active) {
                if (dbEvent.data!.snapshot.exists) {
                  ...
                  numberOfGroupMembers = dataMap["groupMembers"].length;
                  return SingleChildScrollView(
                    child: Column(
                        ...
                        children: [
                          ...
                          Padding(
                            padding: EdgeInsets.only(left: MediaQuery.of(context).size.width * 8/100, bottom: 20, right: MediaQuery.of(context).size.width * 8/100, top: 20),
                            child: SizedBox(
                              height: 50,
                                ...
                                scrollDirection: Axis.horizontal, itemBuilder: (BuildContext context, int index)
                              {
                                return Card(
                                  color: (
                                      (dataMap["buzzer"]["buzzer1"] == widget.uid && widget.studentsPhotos.keys.elementAt(index) == dataMap["buzzer"]["buzzer2"] )
                                          || (dataMap["buzzer"]["buzzer1"] == widget.uid && widget.studentsPhotos.keys.elementAt(index) == dataMap["buzzer"]["buzzer1"]
                                  ...
                                  child: SizedBox(
                                    ...
                                    child: Padding(
                                      padding: const EdgeInsets.all(8.0),
                                      child: Image.network(widget.studentsPhotos.values.elementAt(index)["photoUrl"]),
                                    ),
                                  ),
                                );
                                ...
                                FirebaseDatabase.instance
                                  ...
                                  FirebaseFirestore.instance
                                    ...
                                    if (tempMap["CompletedQuiz"] != null) {
                                      ...
                                      tempList1.add("${widget.testCode}_${widget.groupCode}");
                                      ...
                                      FirebaseFirestore.instance
                                          .collection('StudentsDetails')
                                          .doc(widget.uid).update(
                                          {"CompletedQuiz.${widget.subjectCode}": ["${widget.testCode}"+"_"+"${widget.groupCode}"]
                                          });
                                    }
                                  }).then((_){

                                    FirebaseDatabase.instance
                                        .ref("quizRoom/${widget.proUid}/${widget.subjectCode}/${widget.testCode}/${widget.groupCode}")
                                        .get().then((value){
                                      ...
                                      tempInnerMap01.addAll(tempInnerMap["groupMembers"] as Map<dynamic,dynamic>);
                                      for(int i =0;i<tempInnerMap01.keys.length;i++ ){

                                        if(tempInnerMap01.keys.elementAt(i) == widget.uid){
                                          tempInnerMap01.values.elementAt(i)["score"]=scr;
                                        }
                                      }
                                      tempInnerMap["groupMembers"] = tempInnerMap01;
                                      FirebaseDatabase.instance
                                          .ref("quizRoom/${widget.proUid}/${widget.subjectCode}/${widget.testCode}/${widget.groupCode}")
                                          .update(tempInnerMap.cast<String, Object>());
